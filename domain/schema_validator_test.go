...
func (*testSuite) TestSchemaValidator(c *C) {
	...
	for i := 0; i < 10; i++ {
		...
		item := <-leaseGrantCh
		validator.Update(item.leaseGrantTS, item.oldVer, item.schemaVer, relatedSchemaChange{})
	}

	// Take a lease, check it's valid.
	item := <-leaseGrantCh
	validator.Update(item.leaseGrantTS, item.oldVer, item.schemaVer,
		relatedSchemaChange{dbIDs: []int64{10}, phyTblIDS: []int64{10}, actionTypes: []uint64{10}})
	_, valid := validator.Check(item.leaseGrantTS, item.schemaVer, []int64{10}, false)
	c.Assert(valid, Equals, ResultSucc)

	// Stop the validator, validator's items value is nil.
	validator.Stop()
	c.Assert(validator.IsStarted(), IsFalse)
	_, isTablesChanged := validator.isRelatedTablesChanged(item.schemaVer, []int64{10}, false)
	...
	_, valid = validator.Check(ts, item.schemaVer, []int64{10}, false)
	...
	validator.Update(newItem.leaseGrantTS, newItem.oldVer, currVer, relatedSchemaChange{})
	_, valid = validator.Check(ts, item.schemaVer, nil, false)
	c.Assert(valid, Equals, ResultFail, Commentf("currVer %d, newItem %v", currVer, item))
	_, valid = validator.Check(ts, item.schemaVer, []int64{0}, false)
	c.Assert(valid, Equals, ResultFail, Commentf("currVer %d, newItem %v", currVer, item))
	...
	validator.Update(ts, currVer, newItem.schemaVer, relatedSchemaChange{dbIDs: []int64{1, 2, 3}, phyTblIDS: []int64{1, 2, 3}, actionTypes: []uint64{1, 2, 3}})
	// Make sure the updated table IDs don't be covered with the same schema version.
	validator.Update(ts, newItem.schemaVer, newItem.schemaVer, relatedSchemaChange{})
	_, isTablesChanged = validator.isRelatedTablesChanged(currVer, nil, false)
	c.Assert(isTablesChanged, IsFalse)
	_, isTablesChanged = validator.isRelatedTablesChanged(currVer, []int64{2}, false)
	...
	_, isTablesChanged = validator.isRelatedTablesChanged(-1, nil, false)
	c.Assert(isTablesChanged, IsTrue, Commentf("currVer %d, newItem %v", currVer, newItem))

	// All schema versions is expired.
	ts = uint64(time.Now().Add(2 * lease).UnixNano())
	_, valid = validator.Check(ts, newItem.schemaVer, nil, false)
	c.Assert(valid, Equals, ResultUnknown)
...
func (*testSuite) TestEnqueue(c *C) {
	...
	validator.enqueue(1, relatedSchemaChange{[]int64{11}, []int64{11}, []uint64{11}})
	c.Assert(validator.deltaSchemaInfos, HasLen, 0)

	// maxCnt is 10.
	variable.SetMaxDeltaSchemaCount(10)
	ds := []deltaSchemaInfo{
		{0, []int64{1}, []int64{1}, []uint64{1}},
		{1, []int64{1}, []int64{1}, []uint64{1}},
		{2, []int64{1}, []int64{1}, []uint64{1}},
		{3, []int64{1, 1}, []int64{2, 2}, []uint64{2, 2}},
		{4, []int64{1}, []int64{2}, []uint64{2}},
		{5, []int64{1, 1}, []int64{1, 4}, []uint64{1, 4}},
		{6, []int64{1, 1}, []int64{1, 4}, []uint64{1, 4}},
		{7, []int64{1, 1, 1}, []int64{3, 1, 3}, []uint64{3, 1, 3}},
		{8, []int64{1, 1, 1}, []int64{1, 2, 3}, []uint64{1, 2, 3}},
		{9, []int64{1, 1, 1}, []int64{1, 2, 3}, []uint64{1, 2, 3}},
	}
	for _, d := range ds {
		validator.enqueue(d.schemaVersion, relatedSchemaChange{d.relatedSchemaIDS, d.relatedIDs, d.relatedActions})
	}
	validator.enqueue(10, relatedSchemaChange{[]int64{1}, []int64{1}, []uint64{1}})
	ret := []deltaSchemaInfo{
		{0, []int64{1}, []int64{1}, []uint64{1}},
		{2, []int64{1}, []int64{1}, []uint64{1}},
		{3, []int64{1, 1}, []int64{2, 2}, []uint64{2, 2}},
		...
		{6, []int64{1, 1}, []int64{1, 4}, []uint64{1, 4}},
		{9, []int64{1, 1, 1}, []int64{1, 2, 3}, []uint64{1, 2, 3}},
		{10, []int64{1}, []int64{1}, []uint64{1}},
	}
	c.Assert(validator.deltaSchemaInfos, DeepEquals, ret)
	// The Items' relatedTableIDs have different order.
	validator.enqueue(11, relatedSchemaChange{[]int64{1, 1, 1, 1}, []int64{1, 2, 3, 4}, []uint64{1, 2, 3, 4}})
	validator.enqueue(12, relatedSchemaChange{[]int64{1, 1, 1, 1, 1}, []int64{4, 1, 2, 3, 1}, []uint64{4, 1, 2, 3, 1}})
	validator.enqueue(13, relatedSchemaChange{[]int64{1, 1, 1, 1, 1}, []int64{4, 1, 3, 2, 5}, []uint64{4, 1, 3, 2, 5}})
	ret[len(ret)-1] = deltaSchemaInfo{13, []int64{1, 1, 1, 1, 1}, []int64{4, 1, 3, 2, 5}, []uint64{4, 1, 3, 2, 5}}
	c.Assert(validator.deltaSchemaInfos, DeepEquals, ret)
	// The length of deltaSchemaInfos is greater then maxCnt.
	validator.enqueue(14, relatedSchemaChange{[]int64{1}, []int64{1}, []uint64{1}})
	validator.enqueue(15, relatedSchemaChange{[]int64{1}, []int64{2}, []uint64{2}})
	validator.enqueue(16, relatedSchemaChange{[]int64{1}, []int64{3}, []uint64{3}})
	validator.enqueue(17, relatedSchemaChange{[]int64{1}, []int64{4}, []uint64{4}})
	ret = append(ret, deltaSchemaInfo{14, []int64{1}, []int64{1}, []uint64{1}})
	ret = append(ret, deltaSchemaInfo{15, []int64{1}, []int64{2}, []uint64{2}})
	ret = append(ret, deltaSchemaInfo{16, []int64{1}, []int64{3}, []uint64{3}})
	ret = append(ret, deltaSchemaInfo{17, []int64{1}, []int64{4}, []uint64{4}})
...
func (*testSuite) TestEnqueueActionType(c *C) {
	...
	// maxCnt is 0.
	variable.SetMaxDeltaSchemaCount(0)
	validator.enqueue(1, relatedSchemaChange{[]int64{1}, []int64{11}, []uint64{11}})
	c.Assert(validator.deltaSchemaInfos, HasLen, 0)
	...
	ds := []deltaSchemaInfo{
		...
		{5, []int64{1, 1}, []int64{1, 4}, []uint64{1, 4}},
		{6, []int64{1, 1}, []int64{1, 4}, []uint64{1, 4}},
		{7, []int64{3, 1, 3}, []int64{3, 1, 3}, []uint64{3, 1, 3}},
		{8, []int64{1, 1, 3}, []int64{1, 2, 3}, []uint64{1, 2, 3}},
		{9, []int64{1, 1, 3}, []int64{1, 2, 3}, []uint64{1, 2, 4}},
	}
	for _, d := range ds {
		validator.enqueue(d.schemaVersion, relatedSchemaChange{d.relatedSchemaIDS, d.relatedIDs, d.relatedActions})
	}
	validator.enqueue(10, relatedSchemaChange{[]int64{1}, []int64{1}, []uint64{15}})
	ret := []deltaSchemaInfo{
		{0, []int64{1}, []int64{1}, []uint64{1}},
		{2, []int64{1}, []int64{1}, []uint64{1}},
		{3, []int64{1, 1}, []int64{2, 2}, []uint64{2, 2}},
	...
	relatedChanges, isTablesChanged := validator.isRelatedTablesChanged(5, []int64{1, 2, 3, 4}, true)
	c.Assert(isTablesChanged, Equals, true)
	c.Assert(relatedChanges.dbIDs, DeepEquals, []int64{1, 1, 3, 1})
