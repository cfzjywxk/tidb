...
func (s *testSuite) TestSchemaCheckerSimple(c *C) {
	...
	validator.Update(ts, 0, 2, relatedSchemaChange{dbIDs: []int64{1}, phyTblIDS: []int64{1}, actionTypes: []uint64{1}})
	validator.Update(ts, 2, 4, relatedSchemaChange{dbIDs: []int64{2}, phyTblIDS: []int64{2}, actionTypes: []uint64{2}})

	// checker's schema version is the same as the current schema version.
	checker.schemaVer = 4
	_, _, _, _, err := checker.Check(ts, false)
	c.Assert(err, IsNil)
	...
	_, result := checker.SchemaValidator.Check(nowTS, checker.schemaVer, checker.relatedTableIDs, false)
